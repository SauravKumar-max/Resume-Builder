...
export function Work(){
    const [ work, setWork ] = useState({ id: "", job: "", company: "", startDate: "", endDate: "" });
    const { job, company, startDate, endDate  } = work;
    const { detailState:{ workExperience }, dispatchDetail } = useDetails();
    ...
    return(
        <div className="flex flex-col items-center justify-center mb-8 p-5 border-2 border-coolGray rounded shadow-xl">
            <h2 className="text-2xl font-bold text-center text-black mb-5">Work Experience</h2>
                    ...
                    workExperience?.map(({id, job, startDate, endDate}) => {
                        return(
                            <div key={id} className="flex items-center justify-between my-2 px-5 py-2 border-2 border-coolGray rounded shadow">
                                ...
                                <button onClick={() => dispatchDetail({ type: "REMOVE_WORK_EXPERIENCE", payload: id })}>
                                    <ImCross/>
                                </button>
                            </div>
                        )
            ...
            <div className="w-full flex flex-col md:flex-row items-center justify-center">
                <div className="w-full flex flex-col m-4">
                    ...
                    <input className={inputStyle}
                        value={ job }
                        type="text"
                        onChange={(e) => setWork(work => ({ ...work, job: e.target.value, id: Math.random() }))}
                    />
                </div>
                ...
                <div className="w-full flex flex-col m-4">
                    ...
                    <input className={inputStyle} 
                        value={company}
                        type="text"
                        onChange={(e) => setWork(work => ({ ...work, company: e.target.value }))}
                    />
                </div>
            </div>
            ...
            <div className="w-full flex flex-col md:flex-row items-center justify-center">
                <div className="w-full flex flex-col m-4">
                    ...
                    <input className={inputStyle}
                        type="month" 
                        min="1960-01"
                        value={ startDate ? startDate : getCurrentYear() } 
                        onChange={(e) => setWork(work => ({ ...work, startDate: e.target.value }))}
                    />
                </div>
                ...
                <div className="w-full flex flex-col m-4">
                    ...
                    <input className={inputStyle}
                        type="month" 
                        min="1960-01"
                        value={ endDate ? endDate : getCurrentYear() } 
                        onChange={(e) => setWork(work => ({ ...work, endDate: e.target.value }))}
                    />
                </div>
            </div>
